...
> **v2 (2026-07)**: the workflow was rebuilt around a single planning artifact (`plan.md`) + codegen (`npm run gen`) + scripted verification (`npm run check`), with VO-First (pre-recorded narration) as the primary path and parallel chapter development by default. [`SKILL.md`](./SKILL.md) is the source of truth; this README's workflow narrative predates v2.
...
- [PLAN-FORMAT.md](./references/PLAN-FORMAT.md) — spec for `plan.md` (the single planning artifact) + fact-verification protocol
- [CRAFT.md](./references/CRAFT.md) — per-chapter development rules: hard constraints, beat formula, safe-zone geometry, QA
- [SCRIPT-STYLE.md](./references/SCRIPT-STYLE.md) — article-to-narration rewrite guidance (TTS path)
- [THEMES.md](./references/THEMES.md) — token contract + theme creation workflow
- [AUDIO.md](./references/AUDIO.md) — narration synthesis workflow (TTS path, provider-agnostic)
