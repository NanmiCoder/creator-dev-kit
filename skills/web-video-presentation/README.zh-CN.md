...
> **v2（2026-07）**：工作流已重构为「唯一规划文档 `plan.md` + 代码生成 `npm run gen` + 脚本化校验 `npm run check`」，主路径是 VO-First（先录口播再做画面），章节默认全并行开发。以 [`SKILL.md`](./SKILL.md) 为准；本 README 的流程叙述早于 v2。
...
- [PLAN-FORMAT.md](./references/PLAN-FORMAT.md)：`plan.md` 规格（唯一规划产物）+ 事实核实协议
- [CRAFT.md](./references/CRAFT.md)：每章开发规则——硬约束、节拍公式、安全区几何、QA
- [SCRIPT-STYLE.md](./references/SCRIPT-STYLE.md)：文章转口播稿规则（TTS 路径）
- [THEMES.md](./references/THEMES.md)：token 契约 + 主题创作流程
- [AUDIO.md](./references/AUDIO.md)：口播音频合成流程（TTS 路径，provider-agnostic）
