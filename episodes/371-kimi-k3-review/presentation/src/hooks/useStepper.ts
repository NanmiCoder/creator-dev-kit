 ...
 */
const STORAGE_KEY = "kimi-k3-presentation-cursor-v2";
...
export function useStepper(chapters: ChapterDef[]): StepperState {
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      // In VO-First auto mode, the first Space only unlocks the audio gate.
      // Do not let the same key event advance past the opening frame.
