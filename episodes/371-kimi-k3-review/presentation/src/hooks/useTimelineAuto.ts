...
interface Options {
  ...
  /** Optional chapter-preview stop point; the source audio itself remains untouched. */
  endAtSec?: number;
}
...
export function useTimelineAuto({
  ...
  useEffect(() => {
    ...
    const tick = () => {
      if (endAtSec !== undefined && audio.currentTime >= endAtSec) {
        audio.pause();
        return;
      }
  ...
  }, [enabled, autoStarted, src, endAtSec]);
}
