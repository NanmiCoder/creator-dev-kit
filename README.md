...
这里公开归档视频中实际使用过的网页演示、口播脚本、音频和 SRT 字幕，以及模型实测用的完整提示词与产出代码。大多数期数是独立网页 PPT：进入对应目录安装依赖，就能查看手动模式或音频驱动模式；第 368 期只保留视频中的 Three.js demo，第 389 和 392 期是模型实测归档。
...
第 389 和 392 期是实测归档而非网页 PPT。第 389 期按 case 保存提示词、产出代码与 PerceptionBench 数据；第 392 期让两个模型执行相同的两道全栈题，保存原始英文提示词、四份可运行实现与额外黑盒压力测试报告。
