...
这里公开归档视频中实际使用过的网页演示、口播脚本、音频和 SRT 字幕，以及模型实测用的完整提示词与产出代码。大多数期数是独立网页 PPT：进入对应目录安装依赖，就能查看手动模式或音频驱动模式；第 368 期只保留视频中的 Three.js demo，第 389、392 和 393 期是模型实测归档。
...
| 392 | [Claude Fable 5.1 vs GPT-5.6-SOL 全栈实测](./episodes/392-claude-fable-5-1-vs-gpt-5-6-sol/) | 2 道全栈题、4 份模型产出源码、原始英文提示词与逐项压力测试报告 | 无音轨（实测归档，非网页 PPT） |
| 393 | [GPT-ASTRA 全栈实测](./episodes/393-gpt-astra-fullstack/) | 2 道全栈题的完整提示词、GPT-ASTRA 案例源码、自动化测试与运行说明 | 无音轨（实测归档，非网页 PPT） |
...
第 389、392 和 393 期是实测归档而非网页 PPT。第 389 期按 case 保存提示词、产出代码与 PerceptionBench 数据；第 392 期让两个模型执行相同的两道全栈题，保存原始英文提示词、四份可运行实现与额外黑盒压力测试报告；第 393 期归档 GPT-ASTRA 的响应式表格和多机器人仓库案例，提供完整提示词、源码和复现步骤。
...
第 392 期的四份全栈实现分别独立运行：进入任意模型目录执行 `npm ci && npm run dev`。默认端口和项目专用测试命令见[第 392 期 README](./episodes/392-claude-fable-5-1-vs-gpt-5-6-sol/README.md)。

第 393 期要求 Node.js 22.12+：进入任意 case 下的 `gpt-astra/`，执行 `npm ci && npm run dev`。表格开发页面使用 5173 端口，仓库使用 3000 端口；生产启动、交互复现与验证记录见[第 393 期 README](./episodes/393-gpt-astra-fullstack/README.md)。
